...
export function buildAdvancedDomainModel(domain, state) {
  ...
  const records = rows.map((row, index) => ({ id: row.id || `quote-${index + 1}`, label: row.label, status: row.approved ? 'consented' : row.value ? 'review' : 'missing', score: Number(row.score || 0), owner: getText('owner-reviewer', 'Volta reviewer'), evidence: row.value || 'Consent and quote evidence required', consentRequired: true, channels: ['Website', 'Social', 'Video script'] }));
  return { model: 'Founder story CRM', primaryOutput: 'consented quote bank and multi-channel story package', dashboards: ['Consent coverage', 'Quote quality', 'Story arc completion'], workflows: ['Interview guide', 'Consent capture', 'Quote tagging', 'Channel export'], records, automationRules: ['Block export when consent is missing', 'Flag sensitive topics for mentor review'], enterpriseReadiness: calc.releaseReady && approvedRows === rows.length };
}
